...
def TransformVector(vector, num_utts, plda_W, plda_b, plda_SB, simple_length_norm, normalize_length):
  dim = len(vector)
  normalization_factor = 0.0
  transformed_vector = np.dot(vector - plda_b, plda_W)
  if simple_length_norm:
    normalization_factor = math.sqrt(dim) / np.linalg.norm(transformed_vector)
  else:
    normalization_factor = GetNormalizationFactor(transformed_vector, num_utts, plda_SB)
  if normalize_length:
...
def score_by_trials(enroll_npz, enroll_num_utts, test_npz, test_trials, score_file, plda_W, plda_b, plda_SB, simple_length_norm, normalize_length):
    '''
    ...
    for i in range(len(target_id)):
        ...
        enroll_trans_vec = TransformVector(enroll_vec, enroll_num, plda_W, plda_b, plda_SB, simple_length_norm, normalize_length)
        test_vec = np.array(test_spk2utt[test_id[i]])
        test_trans_vec = TransformVector(test_vec, 1, plda_W, plda_b, plda_SB, simple_length_norm, normalize_length)
        score = NLScore(enroll_trans_vec, enroll_num, test_trans_vec, plda_SB, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--plda-W', default='plda_W', help='file of transform matrix')
    parser.add_argument(
        '--plda-b', default='plda_b', help='file of global bias')
    parser.add_argument(
        '--plda-SB', default='plda_SB', help='file of between-class variance')
    parser.add_argument(
    ...
    plda_W = np.loadtxt(args.plda_W, dtype=np.float).T # W transpose
    plda_b = np.loadtxt(args.plda_b, dtype=np.float)
    plda_SB = np.loadtxt(args.plda_SB, dtype=np.float)
    
    # check model params
    assert(len(plda_b)==len(plda_W))
    assert(len(plda_b)==len(plda_SB))

    score_by_trials(args.enroll_npz, args.enroll_num_utts, args.test_npz, args.trials, args.score, plda_W, plda_b, plda_SB, args.simple_length_norm, args.normalize_length)
