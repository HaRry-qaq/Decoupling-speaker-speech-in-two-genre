...
class ECAPA_TDNN(nn.Module):
    def __init__(self, embedding_dim , pooling_type="ASP", n_mels=80, channels=512, **kwargs):
        ...
        cat_channels_after = cat_channels+128
        ...
        if pooling_type == "Temporal_Average_Pooling" or pooling_type == "TAP":
            ...
            self.pooling = Attentive_Statistics_Pooling(cat_channels_after)
            self.bn1 = nn.BatchNorm1d(cat_channels_after * 2)
            self.linear = nn.Linear(cat_channels_after * 2, embedding_dim)
            self.bn2 = nn.BatchNorm1d(embedding_dim)
    ...
    def forward(self, x,y):
        ...
        y = y.expand(out.size(0),128,out.size(2))
        # print('out:',out.shape)
        # print('y:',y.shape)
