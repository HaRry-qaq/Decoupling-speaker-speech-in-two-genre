...
class Model(LightningModule):
    ...
    def forward(self, x, spk_label, gener_label):
        ...
        s = x.reshape(-1, self.hparams.nPerSpeaker, self.hparams.embedding_dim)
        spk_loss, spk_acc = self.loss(s, spk_label)

        g = self.gener_classifier(x,self.hparams.constant)
        gener_loss = self.gener_criterion(g,gener_label)

        loss = spk_loss + self.hparams.theta * gener_loss
        
        return loss.mean(),spk_acc,spk_loss.mean(),gener_loss.mean()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss,acc,spk_loss,gener_loss = self(data,spk_label,gener_label)
